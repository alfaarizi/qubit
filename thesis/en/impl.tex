...
The platform integrates over thirty libraries across presentation, application, and development layers, each selected to address specific technical challenges in the development of Qubit. This subsection documents major dependencies alongside the rationale guiding their selection.
...
Development quality assurance uses pytest~\cite{pytest2024} for unit and integration testing, with httpx~\cite{httpx2024} providing async HTTP client capabilities for testing FastAPI endpoints. Structured logging uses \texttt{python-json-logger}~\cite{pythonjsonlogger2024}, producing machine-readable log entries facilitating centralized log aggregation and analysis in production environments.

\subsubsection{Development Toolchain}
\label{subsubsec:impl:dev-tools}

Version control uses Git~\cite{git2024} with a feature branch workflow and conventional commit messages, enabling clear commit history and automated changelog generation. Code quality enforcement uses ESLint~\cite{eslint2024} for JavaScript and TypeScript, catching common errors and enforcing consistent code style. UML diagrams presented in the design chapter were generated from text definitions using PlantUML~\cite{plantuml2024}, enabling version control of diagrams and reducing manual maintenance overhead. MongoDB Compass~\cite{mongodbcompass2024} provides graphical database inspection and query optimization during development, while PhpStorm~\cite{phpstorm2024} and PyCharm~\cite{pycharm2024} serve as the primary development environments for frontend and backend respectively.
...
\begin{itemize}
    ...
    \item \textbf{O(n²)}: BQSKit-Scan—the sliding window requires examining pairs of gates within each window, producing quadratic growth. Remains practical for circuits up to several hundred gates.

    \item \textbf{O(n³)}: BQSKit-Greedy—iterative selection of maximum partitions requires examining combinations of remaining gates at each step, producing cubic growth. Becomes prohibitive beyond approximately 500 gates.
