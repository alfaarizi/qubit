...
Qubitkit cannot simulate quantum circuits locally—all computation requires SSH access to external SQUANDER servers. This fundamental dependency prevents deployment for local educational use or in environments without external network access. While delegating computation to remote servers enables advanced simulation capabilities that browsers cannot provide, it also means the platform is useless without this infrastructure. Local simulation support, even if limited to small circuits, would increase flexibility for offline exploration and educational deployment.
