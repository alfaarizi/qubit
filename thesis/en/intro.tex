...
\begin{enumerate}
    \item \textbf{Interactive frontend}: A React-based single-page application providing drag-and-drop circuit composition, real-time gate placement feedback, QASM code editing with Monaco Editor, and responsive visualizations using D3.js and Plotly. The frontend employs Zustand for state management and Zundo middleware to maintain per-circuit undo/redo histories of up to 50 states.

    \item \textbf{FastAPI backend}: A Python service managing user authentication via JWT tokens with support for Google OAuth, Microsoft Azure AD, and email verification. The backend coordinates job submissions, maintains WebSocket connections for progress streaming, and interfaces with MongoDB for persistent storage of user projects and circuit definitions.
