...
The Qubit platform operates as a web application accessible through standard web browsers without local software installation. Users access the system by navigating to the deployed application URL or by setting up a local development environment as described in Section~\ref{sec:user:local-setup}.
...
\textbf{Browser Configuration:} For optimal performance, enable third-party cookies for OAuth providers, WebSocket connections for real-time job monitoring, and LocalStorage for session persistence. Pop-up blockers may interfere with Microsoft Azure AD authentication—temporarily disable them if authentication fails. The platform supports English, German, and Hungarian interface languages.

\section{Local Development Setup}
\label{sec:user:local-setup}

Most users will access Qubit through a deployed web instance without requiring local installation. However, developers and contributors extending the platform will need to run it locally. This section guides you through setting up both frontend and backend components in a development environment.

The setup process requires approximately 20-30 minutes for users with Node.js 18+ and Python 3.8+ already installed. The platform architecture separates into frontend and backend components, each requiring independent setup and configuration. The primary challenge involves configuring environment variables correctly—particularly the MongoDB Atlas connection string and SQUANDER SSH credentials. We recommend following the subsections in order, verifying each component works before proceeding to the next.
...
\begin{enumerate}
    ...
    \item Verify WebSocket connection establishes (check Network tab in browser DevTools)
\end{enumerate}
...
Gate placement occurs through drag-and-drop interaction using native HTML drag-and-drop events. The workflow proceeds as follows:
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{docs/screenshots/gates_selected}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{docs/screenshots/gates_grouped}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{docs/screenshots/flat_circuit_view}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{docs/screenshots/nested_circuit_view}}
...
WebSocket messages use JSON format. Example phase update message:
...
Figure~\ref{fig:viz-partition} shows partition-level analysis. The viewer displays individual partition circuits with gate placement statistics. The distribution histogram shows gate count per partition using color-coded bars: red ($> 90\%$ utilization), orange ($> 70\%$), blue ($> 50\%$), and green (efficient).

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.7\textwidth]{docs/screenshots/viz_partition}}
    \caption{Partition viewer and distribution histogram}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{docs/screenshots/composer_page_dark}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.95\textwidth]{docs/screenshots/exported_circuit_view.png}}
...
This section addresses common issues users encounter when working with Qubit. Each entry follows a three-part structure: symptoms you will observe, the underlying cause, and steps to resolve the issue.
...
\begin{enumerate}
    ...
    \item Temporarily allow pop-ups for the Qubit domain
